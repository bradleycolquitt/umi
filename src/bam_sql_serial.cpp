...
void BamDB::index_cluster() {
    ...
    if ((result = sqlite3_exec(conn, "CREATE UNIQUE INDEX cluster_index ON align (cluster)", NULL, NULL, &err_msg)) != SQLITE_OK) {
        fprintf(stderr, "SQL error (%d) on cluster index: %s \n", result, err_msg);
    }
}
...
int process_read1(BamDB* bamdb, dbRecordPe* record ,bam1_t* b) {
    // #ifdef DEBUG
...
void process_read2(BamDB* bamdb, dbRecordPe* record, bam1_t* b) {
    // #ifdef DEBUG
    // cerr << "process_read2" << endl;
    // #endif //DEBUG
    //DEBUG_LOG("process read2");
    record->split_qname(b);
    if (record->exists(bamdb)) {
        DEBUG_LOG("read exists");
        record->set_positions(b, 2);
        record->set_insert(b);
...
//             record->set_tid(tid);
//             fill_db_tid1(bamdb, record, bam_itr);
//             delete record;
//         }
...
int pass_tid(BamDB* bamdb, dbRecordPe* record, hts_itr_t* bam_itr, int pass_num) {
    bam1_t* b = bam_init1();
    int result;
    ...
    DEBUG_LOG("module pass number:" << pass_num);
    while ((result = sam_itr_next(bamdb->get_bam(), bam_itr, b)) >= 0) {
        if (b->core.flag&BAM_FMUNMAP) continue;
        if ((b->core.flag&BAM_FREAD2) == pass_num)
            process_read2(bamdb, record, b);
        else process_read1(bamdb, record, b);
    }
    bam_destroy1(b);
...
void pass(BamDB* bamdb, int pass_num) {
    hts_itr_t* bam_itr;
    for (int tid = 0; tid < bamdb->get_header()->n_targets; ++tid) {
        bam_itr = bam_itr_queryi(bamdb->get_idx(), tid, 0, bamdb->get_rlen(tid));
        dbRecordPe* record = new dbRecordPe(bamdb);
        record->set_tid(tid);
        DEBUG_LOG("pass number:" << pass_num);
        pass_tid(bamdb, record, bam_itr, pass_num%2); // 1 is read1, 0 is read2
        // if (pass_num == 1) {
        //     pass_tid(bamdb, record, bam_itr);
        // } else if (pass_num == 2) {
        //     fill_db_tid2(bamdb, record, bam_itr);
        // }
        delete record;
        bam_itr_destroy(bam_itr);
    }
...
int fill_db_serial(BamDB* bamdb) {
char* err_msg = 0;
    //turn off synchronous writing to disk for increased insertion speed
    sqlite3_exec(bamdb->get_conn(), "PRAGMA synchronous = OFF", NULL, NULL, &err_msg);
